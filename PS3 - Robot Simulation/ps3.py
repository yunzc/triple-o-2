...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.height = height 
        self.tiles_status = {}
        #Initiate a dictionary that stores the amount of dirt for each tile 
        for horiztile in range(width):
            for vertile in range(height):
                self.tiles_status[(horiztile,vertile)] = dirt_amount
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        dirt_at_pos = self.tiles_status[tile]
        #if capacity exceeds the amount of dirt on the tile, mark as 0
        if capacity > dirt_at_pos:
            self.tiles_status[tile] = 0
        else:
            #clean by removing the capacity amount of dirt 
            self.tiles_status[tile] -= capacity
    ...
    def is_tile_cleaned(self, m, n):
        ...
        """
        position = (m,n)
        #check the amount of dirt and see if it is 0 (0 means clean)
        return self.tiles_status[position] == 0
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        num_clean_tiles = 0
        #tally up the number of cleaned tiles 
        for tile in self.tiles_status:
            #loop through tiles and see if each tile is clean or not 
            if self.tiles_status[tile] == 0:
                num_clean_tiles += 1
    ...
    def is_position_in_room(self, pos):
        ...
        x_lim = self.width
        y_lim = self.height
        #x,y has to by in box [0,width]x[0,height]
        return x_coord < x_lim and y_coord < y_lim and x_coord >= 0 and y_coord >=0
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.position = Position(room.width*random.random(),room.height*random.random())
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        """
        return len(self.tiles_status) - len(self.furniture_tiles)
        
    def get_random_position(self):
        """
        Returns: a Position object; a valid random position (inside the room and not in a furnished area).
        """
        #note all valid tiles
        valid_tiles = []
        for tile in self.tiles_status:
            if tile not in self.furniture_tiles:
                valid_tiles.append(tile)
        #randomly choose a tile out of the valid tiles 
        randomindex = random.randint(0,len(valid_tiles)-1)
        randomtile = valid_tiles[randomindex]
        #random position on tile 
        return Position(randomtile[0]+random.random(), randomtile[1]+random.random())
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_pos = self.get_robot_position().get_new_position(direction, self.speed)
        #check if new position is valid, if not, rotate to random new direction 
        if self.room.is_position_valid(new_pos): #new position is valid, go forth 
            self.set_robot_position(new_pos)
            self.room.clean_tile_at_position(self.get_robot_position(),self.capacity)
        else: #new position is not valid, new direction, stay stationary and clean 
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        if self.gets_faulty(): #check if robot is faulty at this time 
            new_direction = 360*random.random() #change direction randomly and do not clean 
            self.set_robot_direction(new_direction)
        else: #standard none faulty operation 
            direction = self.get_robot_direction()
            new_pos = self.get_robot_position().get_new_position(direction, self.speed)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        #create the robots
        robots = []
        for number in range(num_robots):
            robots.append(robot_type(room, speed, capacity))
        #time each trial
        trial_time = 0
        while min_coverage*room.get_num_tiles() > room.get_num_cleaned_tiles():
            trial_time += 1
            for robot in robots:
                robot.update_position_and_clean()
        sum_of_all_times += trial_time
    #return average
    return sum_of_all_times/num_trials
...
"""
Standard Robot type performs better than the Faulty Robot since it only turns when it absolutely has to
and still cleans up dirt from the tiles. However its performance is only better by a small margin since 
the probability of being faulty is only 15%. Furthermore, as you increase the number of robots, the 
difference in performace of Standard and Sick Robots decreases they clean faster, allowing fewer time 
steps for Faulty Robot to get faulty. 
"""
#
...
"""
Standard Robot type performs better than the Faulty Robot since it only turns when it absolutely
has to and cleans up dirt from the tiles. Amount of time it takes to complete the tasks 
increases for both robot types both when decreasing or increasing form the aspect ration of 1 
since it allows more chance for the robots to run into walls (square room is the best for the 
robots, less chance of walls). However the increase amount of time it takes to complete 
task is more for the Faulty Robot type since more chance of reaching walls means more time steps, 
and more time steps mean more chance for the Faulty Robots to get faulty. 
"""
#
#
